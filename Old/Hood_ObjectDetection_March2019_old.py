...
class HOG:
    ...
    def describe_and_return_HOGImage(self, image):
        # compute Histogram of Oriented Gradients features
        (hist,hogImage) = feature.hog(image, orientations=self.orientations, pixels_per_cell=self.pixelsPerCell,cells_per_block=self.cellsPerBlock, transform_sqrt=self.normalize, visualize=True)
        hist[hist < 0] = 0
        self.plot_hog(image,hogImage)
        # return the histogram
        ...
        for (x, y, window) in sliding_window(layer, winStep, winDim):
                ...
                if winH == winDim[1] and winW == winDim[0]:
                    ...
                    if counter % 5000 ==0:
                        ...
                        print("[INFO] ProcessID: {} Probability: {}  Loop:  {}".format(self.processID,prob,counter))
...
def test_model(hog,conf,image_Filename,SW):
    ...
    minheight=min(int(conf["max_image_width"]), image.shape[0]) # 0 ==> Rows
    minwidth=min(int(conf["max_image_width"]), image.shape[1])  # 1 ==> Columns
...
if __name__ == '__main__':
    ...
    alreadytrained=0;
    #myJSONFile = os.getcwd() + "\\conf\\TrackBibs2019.json"
    ...
    random_start = random.randrange(0,len(TestFiles))
